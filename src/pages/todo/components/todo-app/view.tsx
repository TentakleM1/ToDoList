...
export const ToDoApp: FC = () => {
  ...
  const todos = useSelector((state: TRootState) => {
    return todoSelectCategory(state, filter);
  });
  const handleFilter: TFilter = (filter) => {
    setFilter(filter);
  };
  ...
  return (
    <StyleToDoApp>
      ...
      {todos.length > 0 && (
        <>
          <Main todos={todos.tasks} />
          <Footer count={todos.length} active={filter} filter={handleFilter} />
        </>
      )}
    </StyleToDoApp>
  );
